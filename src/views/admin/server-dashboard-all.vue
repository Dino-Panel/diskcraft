<template>
  <div style="color: var(--bs-text-default);">
    <section>
      <div class="CardArea">
        <serverInfoBlockVue
          v-for="(server, index) in $root.$children[0].user.servers"
          :key="index"
          :server="server"
        ></serverInfoBlockVue>

        <div class="card" v-if="$root.$children[0].user.servers.length < 1">
...
.card-header {
  background-color: var(--bs-card-header) !important;
}
